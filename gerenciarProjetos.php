<?php
  session_start();
  if (!isset($_SESSION['nome'])) {
    header("Location: ./index.php");
  }
?>
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css" type="text/css">
    <link rel="stylesheet" href="./dist/css/theme.css" type="text/css">
    <link rel="shortcut icon" href="./imagens/icone.ico">
    <title>Nightwind - Gerenciar projeto</title>
    <script src="tinymce/tinymce.min.js"></script>
    <script>
      tinymce.init({
      selector: 'textarea',
      plugins: 'code paste print preview fullpage searchreplace autolink directionality visualblocks visualchars fullscreen image link media codesample table charmap hr nonbreaking toc insertdatetime advlist lists textcolor wordcount imagetools contextmenu colorpicker textpattern',
      toolbar: 'print preview | undo redo | image table codesample link | hr | formatselect | bold italic underline strikethrough forecolor backcolor superscript subscript code | alignleft aligncenter alignright alignjustify | numlist bullist outdent indent | removeformat | fullscreen',
      language: 'pt_BR',
      image_caption: true,
      height: 500,
      oninit : "setPlainText",

      // without images_upload_url set, Upload tab won't show up
      images_upload_url: 'upload.php',

      // override default upload handler to simulate successful upload
      images_upload_handler: function (blobInfo, success, failure) {
          var xhr, formData;
        
          xhr = new XMLHttpRequest();
          xhr.withCredentials = false;
          xhr.open('POST', 'upload.php');
        
          xhr.onload = function() {
              var json;
          
              if (xhr.status != 200) {
                  failure('HTTP Error: ' + xhr.status);
                  return;
              }
          
              json = JSON.parse(xhr.responseText);
          
              if (!json || typeof json.location != 'string') {
                  failure('Invalid JSON: ' + xhr.responseText);
                  return;
              }
          
              success(json.location);
          };
        
          formData = new FormData();
          formData.append('file', blobInfo.blob(), blobInfo.filename());
        
          xhr.send(formData);
      },
      });
    </script>
  </head>
  <body>

    <div id="barra-brasil"><div id="wrapper-barra-brasil"><div class="brasil-flag"><a href="http://brasil.gov.br" class="link-barra">Brasil</a></div><span class="acesso-info"><a href="http://www.servicos.gov.br/?pk_campaign=barrabrasil" class="link-barra" id="barra-brasil-orgao">Serviços</a></span><nav><ul id="lista-barra-brasil" class="list"><li><a href="#" id="menu-icon"></a></li><li class="list-item first"><a href="http://www.simplifique.gov.br" class="link-barra">Simplifique!</a></li><li class="list-item"><a href="http://brasil.gov.br/barra#participe" class="link-barra">Participe</a></li><li class="list-item"><a href="http://brasil.gov.br/barra#acesso-informacao" class="link-barra">Acesso à informação</a></li><li class="list-item"><a href="http://www.planalto.gov.br/legislacao" class="link-barra">Legislação</a></li><li class="list-item last last-item"><a href="http://brasil.gov.br/barra#orgaos-atuacao-canais" class="link-barra">Canais</a></li></ul></nav><span id="brasil-vlibras"><a class="logo-vlibras" href="#"></a><span class="link-vlibras"><img src="//barra.brasil.gov.br/imagens/vlibras.gif">&nbsp;<br>O conteúdo desse portal pode ser acessível em Libras usando o <a href="http://www.vlibras.gov.br">VLibras</a></span></span></div></div>

    <div id="barra-ufsm"><div id="wrapper-barra-ufsm"><div class="ufsm-flag"><a href="http://www.ufsm.br" class="link-barra" style="display: list-item;"> </a></div><ul class="list"><li class="list-item first"><a href="http://site.ufsm.br/alunos/" class="link-barra">Alunos</a></li><li class="list-item"><a href="http://site.ufsm.br/servidores/" class="link-barra">Servidores</a></li><li class="list-item"><a href="http://site.ufsm.br/servicos/webmail" class="link-barra"><img src="data:image/png;base64,"> Webmail</a></li><li class="list-item"><a href="http://site.ufsm.br/servicos/moodle" class="link-barra"><img src="data:image/png;base64,"> Moodle</a></li><li class="list-item last last-item"><a href="http://nte.ufsm.br/zikazero/" class="link-barra">#ZikaZero</a></li></ul></div></div>

    <nav class="navbar navbar-expand-md bg-white border navbar-light" style="padding: 0px">
      <div class="container">
        <a href="http://www.ctism.ufsm.br/"><img src="./imagens/ctism.png" height="50" style="padding-right: 16px"></a>
        <a class="navbar-brand" href="./index.php">
          <img src="./imagens/icone.ico" alt="Logotipo do grupo de pesquisa Nightwind" width="50">
          <b>Nightwind</b>
        </a>
        <button class="navbar-toggler navbar-toggler-right" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
          <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
          <ul class="navbar-nav mr-auto">
            <li class="nav-item">
              <a class="nav-link" href="./index.php">Visualizar projetos</a>
            </li>
            <?php
              if (isset($_SESSION['nome'])) {
                echo '
                  <li class="nav-item">
                    <a class="nav-link" href="./cadastrarProjetos.php">Cadastrar projetos</a>
                  </li>
                ';
                if ($_SESSION['tipo'] == 'Administrador') {
                  echo '
                    <li class="nav-item">
                      <a class="nav-link" href="./administrarUsuarios.php">Administrar usuários</a>
                    </li>
                  ';
                }
              }
            ?>
            <li class="nav-item">
              <a class="nav-link" href="./sobre.php">Sobre</a>
            </li>
            <li class="nav-item">
              <a class="nav-link" href="./experience.php">Experience</a>
            </li>
          </ul>
          <form class="form-inline m-0">
            <?php
              if (isset($_SESSION['nome'])) {
                echo '<h6 style="padding-top: 8px;">', '<strong>', $_SESSION['nome'], '&nbsp</h6>';
                echo'<a class="btn btn-secondary" href="./control/controleUsuario.php?opcao=Sair">Sair</a>';
              } else {
                echo'<a class="btn btn-secondary" href="./acessar.php">Acessar minha conta</a>';
              }
            ?>
          </form>
        </div>
      </div>
    </nav>
    <div class="p-2">
      <div class="container-fluid">
        <div class="row">
          <div class="col-md-12">
            <?php 
              include './model/crudProjeto.php';
              if (isset($_GET['codigo'])) {
                $codigo = $_GET["codigo"];
                $resultado = mostrarProjetoAlterar($codigo);
                if ($resultado) {
                  if (mysqli_num_rows($resultado) <= 0) {
                    ?>
                      <div class="p-3">
                        <div class="row text-center">
                          <div class="col-md-12">
                            <h4 class="pt-3"><span class="text-danger">Erro:</span> Projeto não encontrado!<br>Ele pode ter sido removido ou você não tem permissão de acesso ou existe um erro de digitação na url...<br></h4>
                            <a class="btn btn-lg btn-secondary" href="./index.php">Ir para os projetos</a>
                          </div>
                          <div class="col-md-12">
                            <img class="img" src="./imagens/404.png" alt="Imagem representativa do erro 404" height="300px"> </div>
                          </div>
                        </div>
                      </div>
                    <?php
                  }
                  while ($linha = mysqli_fetch_assoc($resultado)) {
                    $codigoProjeto = $linha["codigoProjeto"];
                    $nome = $linha["nome"];
                    $descricao = $linha["descricao"];
                    $conteudo = $linha['conteudo'];
                    $codigoUsuarioProjeto = $linha['codigoUsuario'];
                    $imagemAntiga = $linha['imagemLocal'];
                    if ($codigoUsuarioProjeto != $_SESSION['codigoUsuarioAtual'] && $_SESSION['tipo'] != 'Administrador') {
                      header("Location: ./index.php");
                    }

                    ?>
                      <div class="p-2">
                        <div class="container-fluid">
                          <div class="row">
                            <div class="col-md-12">
                              <h2 class="mb-4">Editar um projeto</h2>
                              <form method="post" action="./control/controleProjeto.php" enctype="multipart/form-data">
                                <div class="form-group">
                                  <label for="nome">Nome do projeto</label>
                                  <input type="text" class="form-control" placeholder="Nome do projeto" id="nome" name="nome" required="" value="<?php echo $nome; ?>">
                                </div>
                                <div class="form-group">
                                  <label for="descricao">Descrição do projeto</label>
                                  <input type="text" class="form-control" placeholder="Descrição do projeto" id="descricao" name="descricao" required="" value="<?php echo $descricao; ?>">
                                </div>
                                <div class="form-group">
                                  <label for="imagem">Imagem para representar o projeto (recomenda-se proporção 16X9)</label><br>
                                  <input type="file" name="imagem" id="imagem">
                                </div>
                                <div class="form-group">
                                  <label for="conteudo">Conteúdo do projeto</label>
                                  <textarea class="form-control" id="myTextarea" name="conteudo" placeholder="Conteúdo do projeto"><?php echo $conteudo; ?></textarea>
                                  <input type="hidden" name="codigoProjeto" value="<?php echo $codigoProjeto;?>">
                                  <input type="hidden" name="imagemAntiga" value="<?php echo $imagemAntiga;?>">
                                </div>
                                <div>
                                  <button type="submit" class="btn btn-primary" name="opcao" value="Alterar" style="margin: 10px 10px 10px 0px">Alterar</button>
                                  <button type="submit" class="btn btn-danger" name="opcao" value="Excluir" style="margin: 10px">Excluir</button>
                                  <a class="btn btn-secondary" href="./index.php" style="margin: 10px">Cancelar</a>
                                </div>
                              </form>
                            </div>
                          </div>
                        </div>
                      </div>                        
                    <?php
                  }
                }
              } elseif (isset($_GET['codigoParaAprovacao'])) {
                if ($_SESSION['tipo'] != 'Administrador') {
                  header("Location: ./index.php");
                } else {
                  $codigo = $_GET["codigoParaAprovacao"];
                  aprovarProjeto($codigo);
                  ?>
                    <script type="text/javascript">
                      window.location="./index.php";
                    </script>
                  <?php
                }
              } elseif (isset($_GET['codigoParaReprovacao'])) {
                if ($_SESSION['tipo'] != 'Administrador') {
                  header("Location: ./index.php");
                } else {
                  $codigo = $_GET["codigoParaReprovacao"];
                  reprovarProjeto($codigo);
                  ?>
                    <script type="text/javascript">
                      window.location="./index.php";
                    </script>
                  <?php
                }
              }
            ?>
          </div>
        </div>
      </div>
    </div>

    <nav style="padding: 0px;" class="navbar bg-white border fixed-bottom">
      &emsp;Plataforma desenvolvida pelo grupo Nightwind - CTISM - Cezar Augusto Crummenauer
    </nav>

    <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
    <script src="./dist/js/barraGov.js"></script>
    <script src="./dist/js/barraUfsm.js"></script>
  </body>
</html>